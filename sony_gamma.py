...
HDMI_INPUT_LEVEL_MAP = {
    HDMIInputLevel.Standard: (0, 1023),
    HDMIInputLevel.Enhanced: (64, 940),
    HDMIInputLevel.SuperWhite: (0, (940 - 64) * (1023 / (1023 - 64))),
    }
...
def basename_to_conf_file_name(basename):
    ...
    return 'sony_gamma_{}.sconf'.format(basename)

def basename_to_conf_file_name_sonyidt(basename):
    """Add prefix and suffix to filename"""
    return 'sony_gamma_{}.ldt'.format(basename)
...
class GammaCurve():
    ...
    def file_save(self, basename=None, save_all_params=False):
        ...
        idt_file = basename_to_conf_file_name_sonyidt(basename)
        dummyzero = 0
        with open(idt_file, 'wb') as file:
            file.write(bytes.fromhex("LDT".encode("utf-8").hex())) #Header: File-Type
            ...
            file.write(bytes.fromhex("NETWORK".encode("utf-8").hex())) #Header: Connection-Type
            file.write(dummyzero.to_bytes(1, byteorder='little')) #Header: 0-Filler
            file.write(bytes.fromhex("0".encode("utf-8").hex())) #Header: ?
            file.write(dummyzero.to_bytes(7, byteorder='little')) #Header: 0-Filler
            file.write(bytes.fromhex("1".encode("utf-8").hex())) #Header: Gamma-Memory-Position
            file.write(dummyzero.to_bytes(7, byteorder='little')) #Header: 0-Filler
            file.write(bytes.fromhex("ALL".encode("utf-8").hex())) #Header: ?
            file.write(dummyzero.to_bytes(5, byteorder='little')) #Header: 0-Filler
            file.write(bytes.fromhex("0".encode("utf-8").hex())) #Header: ?
    ...
    def generate_table(self):
        ...
        points = list(map(self.itop, range(1024)))
        clip_p = math.inf
        clip_l = math.inf
        clip_gain = None
        hardclip_p = self.itop(1024) #??
        last_p = None
        pblackin = -math.inf
        for p in points:
            l = ptol(p)
